...
void ConsoleLogger::AddLog(const std::string& message, LogLevel level, LogSource source, LogCategory category,
                           float timestamp) {
...
ImVec4 ConsoleLogger::GetCategoryColor(LogCategory category) {
    switch (category)
    {
        ...
        case LogCategory::RENDER:
            return ImVec4{1.0f, 0.4f, 0.6f, 1.0f};
        case LogCategory::SCENE:
            return ImVec4{0.6f, 1.0f, 1.0f, 1.0f};
        ...
        case LogCategory::BUILD:
            return ImVec4{0.7f, 0.7f, 0.4f, 1.0f};
